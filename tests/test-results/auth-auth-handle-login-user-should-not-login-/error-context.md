...
- Location: /home/ninja/Documents/PROJECTS/PERN__BOOK/tests/src/e2e/auth/auth.test.ts:124:3
...
```ts
   ...
   28 | });
   29 |
   30 | // test.describe("handle non existent user", () => {
   31 | //   test("should not login", async ({ page }) => {
   32 | //     await nav(page, "auth/login");
   33 |
   34 | //     await page.getByRole("textbox", { name: "email" }).fill(account_0.email);
   35 | //     await page.getByLabel("password").fill(account_0.pwd);
