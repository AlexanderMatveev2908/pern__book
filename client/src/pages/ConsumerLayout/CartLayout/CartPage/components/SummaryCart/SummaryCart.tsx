...
const SummaryCart: FC<PropsType> = ({ groupedByStoreID }) => {
  ...
  return !isArrOk(cart?.items) ? null : (
    <div className="sticky top-[85px] p-4 pt-0 border-[3px] border-blue-600 rounded-xl grid grid-cols-1 gap-y-4 z-60 bg-neutral-950 w-full max-w-[1000px] ">
      ...
      <div className="w-full grid grid-cols-1 gap-y-4 max-h-[200px] overflow-y-auto scroll_app scroll_y">
        {Object.entries(groupedByStoreID).map(
          ([_, { store, items }], outerI) => (
            <div
              key={ids[0][outerI]}
              className="w-full grid grid-cols-1 gap-y-6 justify-items-center pl-3 border-l-2 border-blue-600"
            >
